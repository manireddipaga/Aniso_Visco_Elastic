...
% close all
% clc
function stress_stretch_plot(x)
%% Experimental Data
load per90.mat per9010 per901 per9001 per90001
load time90.mat time1 time2 time3 time4
load khat.mat khat
load kbardata.mat kbardata
% figure()
% plot(per9010(:,1),per9010(:,2),'r',LineWidth=2)
% hold on
% plot(per901(:,1),per901(:,2),'g',LineWidth=2)
% plot(per9001(:,1),per9001(:,2),'b',LineWidth=2)
% plot(per90001(:,1),per90001(:,2),'m',LineWidth=2)
% hold off
% xlabel('\lambda -1')
% ylabel('\sigma')
...
%% Plotting
% figure()
% plot(lda1(time1),deval(sol1,time1))
% figure()
% plot(lda2(time2),deval(sol2,time2))
% figure()
% plot(lda3(time3),deval(sol3,time3))
% figure()
% plot(lda4(time4),deval(sol4,time4))
...
PK1             =       @(t) lda1(t).^(-1).*TAI(deval(sol1,t),lda1(t),binit,mu1,mu2)+(-1).* ...
  deval(sol1,t).*lda1(t).^(-2).*mu01+deval(sol1,t).^(-2).*lda1(t).*mu01;
PK2             =       @(t) lda2(t).^(-1).*TAI(deval(sol2,t),lda2(t),binit,mu1,mu2)+(-1).* ...
  deval(sol2,t).*lda2(t).^(-2).*mu02+deval(sol2,t).^(-2).*lda2(t).*mu02;
PK3             =       @(t) lda3(t).^(-1).*TAI(deval(sol3,t),lda3(t),binit,mu1,mu2)+(-1).* ...
  deval(sol3,t).*lda3(t).^(-2).*mu03+deval(sol3,t).^(-2).*lda3(t).*mu03;
PK4             =       @(t) lda4(t).^(-1).*TAI(deval(sol4,t),lda4(t),binit,mu1,mu2)+(-1).* ...
  deval(sol4,t).*lda4(t).^(-2).*mu04+deval(sol4,t).^(-2).*lda4(t).*mu04;

%% plotting
figure()
subplot(2,2,1)
plot(per9010(:,1),per9010(:,2),'o')
title('10%')
xlabel('\lambda -1')
ylabel('stress')
hold on
plot(lda1(time1)-1,PK1(time1')','r-',LineWidth=3)
hold off
subplot(2,2,2)
plot(per901(:,1),per901(:,2),'o')
title('1%')
xlabel('\lambda -1')
ylabel('stress')
hold on
plot(lda2(time2)-1,PK2(time2')','r-',LineWidth=3)
hold off
subplot(2,2,3)
plot(per9001(:,1),per9001(:,2),'o')
title('0.1%')
hold on
xlabel('\lambda -1')
ylabel('stress')
plot(lda3(time3)-1,PK3(time3')','r-',LineWidth=3)
hold off
subplot(2,2,4)
plot(per90001(:,1),per90001(:,2),'o')
title('0.01%')
xlabel('\lambda -1')
ylabel('stress')
hold on
plot(lda4(time4)-1,PK4(time4')','r-',LineWidth=3)
hold off
%%
